...
from pyqg_generative.tools.cnn_tools import AndrewCNN, ChannelwiseScaler, log_to_xarray, save_model_args, \
    ...
    AverageLoss, upsampling, downsampling
...
class CVAEBottleneck(Parameterization):
    def __init__(self, regression='full_loss', nx=64, decoder_var = 'adaptive', folder='model', div=False):
        '''
        ...
        # Encoder (x,y -> z)
        self.encoder = downsampling(4, n_in+n_out, 2*self.deep_latent, nx=nx)
        self.deep_decoder = upsampling(4, self.deep_latent, self.n_latent, nx=nx)
        if regression != 'None':
    ...
    def encode(self, x, y):
        ...
        mu = result[:,:self.deep_latent]
        logvar = result[:,self.deep_latent:]

        return mu, logvar

    def generate(self, x, z=None):
        dims = (x.shape[0], self.deep_latent)
        if z is None:
    ...
    def compute_loss(self, x, ytrue, ymean):
        ...
        loss_KL = KL_pointwise.sum(dim=(1)).mean()

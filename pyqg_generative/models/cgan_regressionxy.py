...
class CGANRegressionxy(Parameterization):
    def __init__(self, regression='full_loss', nx=64, folder='model'):
        '''
        Generator is assumed to work ass:
        G(x,ymean,z) = CNN(x,ymean,z) + ymean
        i.e. conditioning on both x and ymean
        Discriminator is again conditioned on both x and ymean,
        but can be applied to full subgrid forcing or residual

        Regression parameter:
        'full_loss' - discriminator to full subgrid forcing
        'residual_loss' - discriminator to residual
        '''
        # 2 Input layers of q
        n_in = 2
        ...
        self.G = AndrewCNN(n_in+n_out+self.n_latent,n_out)
        self.D = DCGAN_discriminator(n_in+n_out+2*n_out, bn='None', nx=nx)
    ...
    def save_model(self, optim_loss, log_train, log_test):
        ...
        save_model_args('CGANRegressionxy', regression=self.regression, nx=self.nx)
    ...
    def generate(self, x, ymean, z=None):
        dims = (x.shape[0], self.n_latent, x.shape[2], x.shape[3])
        if z is None:
            z = torch.randn(dims, device=x.device)
        return self.G(torch.cat([x,ymean,z], dim=1))

    def generate_mean_var(self, x, ymean, M):
        y = []
        for m in range(M):
            y.append(self.generate(x, ymean))
    ...
    def predict_snapshot(self, m, noise):
        ...
        Ymean = apply_function(self.net_mean, X)
        Y = apply_function(self.G, X, Ymean, noise, fun=self.generate)
        return self.y_scale.denormalize(Ymean + Y).squeeze().astype('float64')
    
    def predict(self, ds, M=1000):
        X = self.x_scale.normalize(extract(ds, 'q'))
        Ymean = apply_function(self.net_mean, X)
        Y, mean, var = apply_function(self.G, X, Ymean, fun=self.generate_mean_var, M=M)
        Y += Ymean
        mean += Ymean
...
def gradient_penalty(net, xtrue, ymean, ytrue, yfake1, 
    yfake2):
    ...
    D = net.D(torch.cat((xtrue, ymean, yinterp), dim=1))
...
def train_CGAN(net, ds_train, ds_test,
    ...
    for epoch in range(num_epochs):
        ...
        for i, (x,y,y_) in enumerate(minibatch(X_train, Y_train, Y_mean, batch_size=batch_size)):
            xtrue = x.to(device)
            ytrue = y.to(device)
            ymean = y_.to(device)

            net.D.zero_grad()
            yfake1 = net.generate(xtrue, ymean)
            yfake2 = net.generate(xtrue, ymean)

            if net.regression == 'full_loss':
                yfake1 += ymean
                yfake2 += ymean
            elif net.regression == 'residual_loss':
                ytrue -= ymean

            Dtrue1 = net.D(torch.cat([xtrue, ymean, ytrue, yfake2.detach()], dim=1))
            Dtrue2 = net.D(torch.cat([xtrue, ymean, yfake1.detach(), ytrue], dim=1))
            Dfake  = net.D(torch.cat([xtrue, ymean, yfake1.detach(), yfake2.detach()], dim=1))
            D_loss = - 0.5 * (Dtrue1.mean() + Dtrue2.mean()) + Dfake.mean()
            D_drift = LAMBDA_DRIFT * (Dtrue1**2).mean()
            D_grad = gradient_penalty(net, xtrue, ymean, ytrue, yfake1, yfake2)
            ...
            if i % 5 == 0:
                net.G.zero_grad()
                G_loss = - (net.D(torch.cat([xtrue, ymean, yfake1, yfake2], dim=1))).mean()
                G_loss.backward()

...
for resolution in [32, 48, 64, 96]:
    for operator in ['Operator1', 'Operator2']:
        for model, folder in zip(['OLSModel', 'CGANRegression'], ['OLSModel-div', 'CGANRegression-div']):
            ...
            model_args = dict(nx=resolution, div=True) if model=='CGANRegression' else dict(div=True)
            fit_args = {}
